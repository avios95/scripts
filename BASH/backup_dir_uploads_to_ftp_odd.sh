...
for i in $(ls -1 $workdir | grep monthly_`date +%Y_%m`) ; do
    ...
    wput $workdir/$i/ ftp://$userftp:$passftp@$hostftp/upload_odd/ 2>>$logdir/$logfile && echo "Files from $workdir is sended" >> $logdir/$logfile || echo "Copied files from $workdir finished with error"  >> $logdir/$logfile
